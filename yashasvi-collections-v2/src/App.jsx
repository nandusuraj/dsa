...
const OWNER_WHATSAPP = "919930274669";

// --- UTILITY COMPONENTS ---

const Logo = ({ className = "", size = "large" }) => (
  <div className={`flex flex-col items-center ${className}`}>
    <h1 className={`font-serif font-semibold text-[#7B1E2B] tracking-wide leading-none ${size === 'large' ? 'text-5xl md:text-6xl' : 'text-2xl md:text-3xl'}`}>
      यशस्वी
    </h1>
    <span className={`flex items-center gap-2 mt-1.5 ${size === 'large' ? '' : 'mt-1'}`}>
      <span className="h-px w-3 bg-[#C9A227]/60" />
      <span className={`font-serif italic text-[#C9A227] tracking-[0.25em] ${size === 'large' ? 'text-sm md:text-base' : 'text-[10px]'}`}>
        कलेक्शन्स
      </span>
      <span className="h-px w-3 bg-[#C9A227]/60" />
    </span>
...
const Button = ({ children, variant = "primary", className = "", onClick, disabled = false, type = "button" }) => {
  const baseStyles = "px-7 py-3.5 rounded-sm font-medium tracking-wide text-[13px] uppercase transition-all duration-500 ease-out flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed";
  const variants = {
    primary: "bg-[#7B1E2B] text-white hover:bg-[#5C0F1B] shadow-[0_8px_24px_-6px_rgba(123,30,43,0.4)] hover:shadow-[0_12px_28px_-6px_rgba(123,30,43,0.5)] hover:-translate-y-0.5",
    secondary: "bg-[#C9A227] text-white hover:bg-[#b08d1f] shadow-[0_8px_24px_-6px_rgba(201,162,39,0.4)] hover:shadow-[0_12px_28px_-6px_rgba(201,162,39,0.5)] hover:-translate-y-0.5",
    outline: "border border-[#7B1E2B] text-[#7B1E2B] hover:bg-[#7B1E2B] hover:text-white",
    ghost: "text-[#7B1E2B] hover:bg-[#7B1E2B]/10 normal-case tracking-normal text-sm",
    white: "bg-white text-[#7B1E2B] hover:bg-[#FFF8F2] shadow-[0_8px_24px_-6px_rgba(0,0,0,0.25)] hover:-translate-y-0.5"
  };
...
export default function App() {
  ...
  const handleCheckout = () => {
    ...
    const itemLines = cart.map(item =>
      `• ${item.name} (Size: ${item.selectedSize || 'Free Size'}) x${item.quantity} = ₹${item.price * item.quantity}`
    ).join('\n');
    ...
    const message =
      `🛍️ *New Order from Yashasvi Collections*
...
Please confirm my order. Thank you!`;
  ...
  return (
    <div className="min-h-screen bg-[#FFFFFF] font-sans text-[#2D2D2D]">
      {/* Announcement Bar */}
      <div className="bg-[#2A0E13] text-white text-[11px] md:text-xs py-2.5 px-4 tracking-wide border-b border-[#C9A227]/30">
        <div className="max-w-7xl mx-auto flex justify-center md:justify-between items-center">
          <div className="flex gap-6">
            <span className="flex items-center gap-1.5"><Truck size={13} className="text-[#C9A227]" /> Complimentary shipping above ₹999</span>
            <span className="hidden md:flex items-center gap-1.5"><Shield size={13} className="text-[#C9A227]" /> Cash on delivery available</span>
          </div>
          <div className="hidden md:flex gap-6">
            <button className="hover:text-[#C9A227] transition-colors duration-300">Track Order</button>
            <button className="hover:text-[#C9A227] transition-colors duration-300">Help & Support</button>
          </div>
        </div>
      </div>
            ...
            <nav className="hidden md:flex items-center gap-9">
              {['Home', 'Shop', 'Sarees', 'Kurtis', 'Sets', 'T-Shirts', 'Jeans', 'Contact'].map((item) => (
                <button
                  ...
                  className="text-[11px] tracking-[0.12em] uppercase font-medium text-[#2D2D2D] hover:text-[#7B1E2B] transition-colors duration-300 relative group"
                >
                  {item}
                  <span className="absolute -bottom-1.5 left-0 w-0 h-px bg-[#C9A227] transition-all duration-500 group-hover:w-full" />
                </button>
              ))}
            </nav>
    ...
    <div className="animate-in fade-in duration-700">
      ...
      <section className="relative h-[600px] md:h-[700px] overflow-hidden bg-[#F5EBDD]">
        <div className="absolute inset-0">
          <img
            src="/workspaces/dsa/yashasvi-collections-v2/src/assets/hero.png"
            alt="Hero"
    ...
    <div className="max-w-7xl mx-auto px-4 py-12 animate-in fade-in">
      ...
      <div className="grid lg:grid-cols-3 gap-12">
        ...
        <div className="bg-[#FFF8F2] p-8 rounded-2xl h-fit">
          ...
          <button
            onClick={onCheckout}
            className="w-full py-4 rounded-sm font-medium tracking-wide text-[13px] uppercase transition-all duration-500 flex items-center justify-center gap-3 bg-[#25D366] hover:bg-[#1ebe5d] text-white shadow-[0_8px_24px_-6px_rgba(37,211,102,0.5)] hover:shadow-[0_12px_28px_-6px_rgba(37,211,102,0.6)] hover:-translate-y-0.5"
          >
            {/* WhatsApp SVG icon */}
          ...
          <nav className="p-4 space-y-2">
            {[
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${activeTab === tab.id ? 'bg-[#7B1E2B] text-white' : 'text-gray-600 hover:bg-[#F5EBDD]'
                  }`}
              >
                <tab.icon size={18} />
                {tab.label}
...
function ProductForm({ product, onSave, onCancel }) {
  ...
  const [imagePreview, setImagePreview] = useState(product?.image || 'https://images.unsplash.com/photo-1610030469983-98e550d6193c?w=600&q=80');
  ...
  const handleImageUpload = (e) => {
    ...
    if (file.size > 3 * 1024 * 1024) {
      alert("Image is larger than 3MB. It will still work, but consider using a compressed image for better performance.");
    }
    ...
    <div className="bg-white rounded-2xl shadow-lg border border-[#F5EBDD] p-6 mb-8 animate-in slide-in-from-top-4">
      ...
      <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          {/* IMAGE SECTION */}
          <div>
            <label className="block text-sm font-medium text-[#2D2D2D] mb-2">Product Image</label>

            {/* Toggle tabs */}
            <div className="flex rounded-lg border border-[#F5EBDD] overflow-hidden mb-3">
              <button
                type="button"
                onClick={() => setImageMode('url')}
                className={`flex-1 py-2 text-xs font-medium transition-colors ${imageMode === 'url' ? 'bg-[#7B1E2B] text-white' : 'text-gray-500 hover:bg-[#FFF8F2]'}`}
              >
                Paste URL
              </button>
              <button
                ...
                className={`flex-1 py-2 text-xs font-medium transition-colors ${imageMode === 'upload' ? 'bg-[#7B1E2B] text-white' : 'text-gray-500 hover:bg-[#FFF8F2]'}`}
              >
            ...
            {imageMode === 'url' ? (
              <input
                ...
                className="w-full px-4 py-2 rounded-lg border border-[#F5EBDD] focus:border-[#7B1E2B] outline-none text-sm"
              ...
              <div>
                ...
                <button
                  ...
                  className="w-full flex flex-col items-center justify-center gap-2 px-4 py-6 border-2 border-dashed border-[#C9A227] rounded-lg text-[#C9A227] hover:bg-[#C9A227]/5 transition-colors"
                >
                  <Upload size={24} />
                  <span className="text-sm font-medium">Click to choose an image</span>
                  <span className="text-xs text-gray-400">JPG, PNG, WEBP · Max 3MB recommended</span>
                </button>
              </div>
            )}
            ...
            {imagePreview && (
              <div className="mt-3 flex items-start gap-3">
                <div className="relative w-20 h-20 rounded-lg overflow-hidden border border-[#F5EBDD] flex-shrink-0">
                  <img
                    src={imagePreview}
                    alt="Preview"
